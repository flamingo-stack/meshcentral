        ...
        switch (command.action) {
            ...
            case 'dialogResponse': {
                if (command.args.ider == 1) { // Start IDER Session
                    ...
                    var domainx = 'domain' + ((domain.id == '') ? '' : ('-' + domain.id));
                    var useridx = user._id.split('/')[2];
                    var userBase = path.resolve(parent.parent.path.join(parent.parent.filespath, domainx, 'user-' + useridx));
                    var floppyPath = null, cdromPath = null;
                    if (command.args.floppyPath) {
                        var resolvedFloppy = path.resolve(path.join(userBase, command.args.floppyPath));
                        if (resolvedFloppy.indexOf(userBase + path.sep) === 0) { floppyPath = resolvedFloppy; }
                    }
                    if (command.args.cdromPath) {
                        var resolvedCdrom = path.resolve(path.join(userBase, command.args.cdromPath));
                        if (resolvedCdrom.indexOf(userBase + path.sep) === 0) { cdromPath = resolvedCdrom; }
                    }
