    ...
    function completeAgentConnection2() {
        ...
        if (mesh == null) {
            ...
            try {
                ...
                if (!diagSeen.orphaned) { diagSeen.orphaned = true; parent.parent.debug('diag', new Date().toISOString() + ' Agent census: node ' + obj.nodeid + ' -> group ' + obj.dbMeshKey + ' (ORPHANED, no such device group)'); }
            } catch (diagEx) { }
            return;
        }
        ...
        try {
            if (parent.diagAgentSeen == null) { parent.diagAgentSeen = {}; }
            var diagSeen = parent.diagAgentSeen[obj.nodeid] || (parent.diagAgentSeen[obj.nodeid] = {});
            if (!diagSeen.ok) { diagSeen.ok = true; parent.parent.debug('diag', new Date().toISOString() + ' Agent census: node ' + obj.nodeid + ' -> group ' + obj.dbMeshKey + ' (OK, name="' + (mesh.name || '') + '")'); }
        } catch (diagEx) { }
